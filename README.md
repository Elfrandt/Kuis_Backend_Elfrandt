...
Proyek ini adalah sistem backend REST API untuk aplikasi undian dengan sistem _gacha_. Sistem ini memungkinkan pengguna untuk melakukan penarikan undian dengan batasan kuota harian serta kuota hadiah yang tersedia selama satu periode undian. Seluruh riwayat transaksi (baik yang menang maupun tidak) dan data hadiah dikelola menggunakan database MongoDB.

### Aturan Bisnis & Requirement
...
### 2. Fitur Bonus (Bonus Points)
   ...
   Method: GET
   Endpoint: /gacha/history/:userIdURL
   Parameter: - userId: ObjectId dari user yang ingin dilihat historinya.
   Body Request: Tidak ada.
   Response (200 OK): Array of objects berisi tanggal gacha dan nama hadiah.

B. Prize List & Remaining Quota
Menampilkan daftar seluruh hadiah yang tersedia dalam sistem beserta sisa kuota (remaining quota) untuk setiap hadiah.
Method: GET
...
Menampilkan daftar semua user yang berhasil memenangkan hadiah. Nama user disamarkan secara acak demi privasi (misalnya John Doe menjadi J*h* _o_).
Method: GET
